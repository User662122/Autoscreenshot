...
class TFLiteModelManager(private val context: Context) {
    ...
    private val NUM_INTERPRETERS = 8
    private val BATCH_SIZE = 8
    ...
    private fun initializeModel(context: Context) {
        try {
            ...
            for (i in 0 until NUM_INTERPRETERS) {
                interpreters[i] = Interpreter(model, options)
            }
        } catch (e: Exception) {
            e.printStackTrace()
        }
    ...
    private suspend fun classifyAllPieces(pieces: List<Bitmap>): List<String> {
        return withContext(Dispatchers.Default) {
            ...
            val deferredResults = (0 until 64 step BATCH_SIZE).mapIndexed { batchIndex, i ->
                async {
                    val interpreterIndex = batchIndex % NUM_INTERPRETERS
                    val interpreter = interpreters[interpreterIndex]!!

                    for (j in i until minOf(i + BATCH_SIZE, 64)) {
    ...
    private fun sendDataToBackend(context: Context) {
        CoroutineManager.launchIO {
            try {
                ...
                if (fetchSuccess && fetchedMove.isNotEmpty() && fetchedMove != "Invalid" && fetchedMove != "Game Over") {
                    // Move already saved in NetworkManager.fetchPendingMove()
                }
                ...
                if (whiteUCI.isNotEmpty() && blackUCI.isNotEmpty()) {
                    ...
                    if (positionSuccess && positionResponse.isNotEmpty() && positionResponse != "Invalid" && positionResponse != "Game Over") {
